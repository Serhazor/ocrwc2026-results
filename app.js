(() => {
  ...
  const CAROUSEL_LOGO = '/assets/ocra-eireann-logo-2026-transparent-v2.png';
  ...
  function renderNotes() {
    setTitle('Data Notes');
    app.innerHTML=`<div class="section-head"><div><div class="eyebrow">Methodology & caveats</div><h1>Data Notes</h1><p>What is calculated, what is source data, and where the timing export has decided to make life interesting.</p></div></div><div class="grid-2">${D.dataNotes.map(n=>`<div class="note-card ${n.level==='warning'?'warning':''}"><h3>${n.level==='warning'?'⚠️ ':'ℹ️ '}${esc(n.title)}</h3><p>${esc(n.text)}</p></div>`).join('')}</div><section class="section panel"><div class="section-head"><div><h2>Source files</h2><p>The static dataset used by this site was generated from these supplied workbooks.</p></div></div><ul class="source-list">${D.meta.sourceFiles.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section class="section panel"><h2>What the site does not claim</h2><p class="muted">This archive does not silently repair uncertain source records. Calculated medal tables follow the documented methodology above, and any confirmed federation corrections or synthetic placement times are disclosed in these notes. Where the source is incomplete, the UI says so.</p></section>`;
  }
